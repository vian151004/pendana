    ...
    <div class="col-lg-4">
        ...
        <x-card>
            ...
            <div class="progress" style="height: .3rem;">
                <div class="progress-bar" role="progressbar" style="width: {{ $campaign->nominal / $campaign->goal * 100 }}%" 
                    aria-valuenow="{{ $campaign->nominal / $campaign->goal * 100 }}" aria-valuemin="0" aria-valuemax="{{ 100 }}">
                </div>
            </div>
            ...
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-waktu" role="tabpanel"
                    ...
                    @forelse ($campaign->donations->where('status', 'confirmed')->sortBy('created_at')->load('user') as $k => $v)
                        <div @if ($k > 0) class="mt-1" @endif>
                            <p class="font-weight-bold mb-0">{{ $v->user->name }}</p>
                            <p class="font-weight-bold mb-0">Rp. {{ format_uang($v->nominal) }}</p>
                            <p class="text-muted mb-0">{{ tanggal_indonesia($v->created_at) }}</p>
                        </div>
                        @empty
